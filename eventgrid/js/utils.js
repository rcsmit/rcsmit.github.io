 ...
 * Reads from CONFIG.CATEGORIES (populated from JSON) so adding a new
 * category in the JSON automatically shows the right icon everywhere.
 */
function etIcon(t) {
  if (CONFIG.CATEGORIES) {
    const cat = CONFIG.CATEGORIES.find(c => c.label === t || c.id === t);
...
/**
 * Return the accent colour for a given event category from JSON config.
 * Falls back to primary brand colour.
 */
function etColor(t) {
  if (CONFIG.CATEGORIES) {
    const cat = CONFIG.CATEGORIES.find(c => c.label === t || c.id === t);
  ...
  return CONFIG.SL_COLOR || getComputedStyle(document.documentElement).getPropertyValue('--p1').trim() || '#888';
}
 ...
 * Handles: quoted fields, escaped quotes (""), CRLF, LF, and lone CR.
 */
